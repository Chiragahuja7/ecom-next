...
export default function SearchModal({ isOpen, onClose, searchQuery }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      <div className="absolute inset-0 pt-10" onClick={onClose}></div>

      <div className={`absolute top-16 left-0 w-full bg-white shadow-lg transform transition-transform duration-300 ease-in-out ${show ? "translate-y-0" : "-translate-y-full"}`}>
        <div className="p-4">
            ...
            {!loading && results.length > 0 && (
              <div className="mt-3 overflow-x-auto text-black">
                <div className="flex gap-4">
                  {results.map((p) => (
                    <Link href={`/shop/${ p.slug}`} key={p._id || p.id} className="w-48 shrink-0 bg-white border border-gray-300 rounded p-2 ">
